...
if __name__ == '__main__':
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            print(net)
            batch_size=1  # 随便一个数
            x = torch.randn(batch_size,3,300,300)
            #print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
            #torch.onnx.export(net,x,"pre.onnx",verbose=False)
            print("to onnx1111111111111111111")
            torch.save(net.state_dict(),model_path)
            print('11111111111111111')
            model =net.load_state_dict(torch.load(model_path), strict=False)
            print(model)
            torch.onnx.export(model,x,"pre.onnx",verbose=False)
            print('to onnx2222222222222222222222')
