...
class SqueezeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = x.flatten(start_dim=2).mean(dim=2) 
        #F.avg_pool2d(x, kernel_size=[height, width],ceil_mode=False).view(batch, -1)
        #print(out)
        out = self.dense(out)
...
class MobileNetV3(nn.Module):
    ...
    def __init__(self, model_mode="SMALL", num_classes=4, multiplier=1.0, dropout_rate=0.5):
        ...
        if model_mode == "LARGE":
            ...
            #print(type(out_conv1_out),type(out_conv2_out))
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(out, kernel_size=[height, width],ceil_mode=False)
